...
servidor.post("/registros", async (c) => {
  ...
  if (!dados.nome || dados.nome.trim() === "") {
    return c.json({ erro: "Campo de nome \xE9 Obrigat\xF3rio!" }, 400);
  } else if (dados.nome.length > 100 || dados.nome.length < 3) {
    return c.json({ erro: "Nome inv\xE1lido, deve conter entre 3-100 caracteres" }, 400);
  }
  if (!dados.email) {
    return c.json({ erro: "Campo de email \xE9 Obrigat\xF3rio!" }, 400);
  } else if (dados.email.split(".").length < 2 || dados.email.split("@").length < 2) {
    return c.json({ erro: "Email inv\xE1lido!" }, 400);
  }
  if (!dados.senha) {
    return c.json({ erro: "Campo de senha \xE9 Obrigat\xF3rio!" }, 400);
  } else if (dados.senha.length < 7) {
    return c.json({ erro: "Senha inv\xE1lida, deve conter m\xEDnimo de 7 caracteres" }, 400);
  }
  if (!dados.notebookId) {
    return c.json({ erro: "N\xFAmero do notebook inv\xE1lido!" }, 400);
...
servidor.delete("/registros/:id", async (c) => {
  ...
  if (info.meta.changes === 0) {
    return c.json({ erro: "Aluno n\xE3o encontrado!" }, 404);
  }
...
servidor.put("/registros/:id", async (c) => {
  ...
  if (!alunoExiste) {
    return c.json({ erro: "Registro n\xE3o encontrado!" }, 404);
  }
  ...
  if (!dados.turmaId || Number(dados.turmaId) <= 0) {
    return c.json({ erro: "A sele\xE7\xE3o de uma turma \xE9 obrigat\xF3ria!" }, 400);
  }
  const turmaExiste = await c.env.DB.prepare("SELECT id FROM turmas WHERE id = ?").bind(dados.turma_id).first();
  if (!turmaExiste) {
    return c.json({ erro: "A turma selecionada n\xE3o existe no sistema." }, 404);
  }
  try {
    await c.env.DB.prepare("INSERT OR IGNORE INTO notebooks (numero, status) VALUES (?, 'em_uso')").bind(dados.notebookId).run();
...
servidor.post("/dev/seed", async (c) => {
  try {
    ...
    const seedAlunos = db.prepare(`
      ...
      ('Jo\xE3o Silva', 'joao.silva@cerquilha.br', 'senha123', 1, 10),
      ...
      ('Let\xEDcia Ribeiro', 'leticia.ribeiro@cerquilha.br', 'senha123', 4, 21),
...
servidor.get("/metricas", async (c) => {
  try {
    ...
    return c.json({
      ...
      taxaOcupacao,
      limiteNotebooks,
      alunosPorTurno: turnos,
      // O Hono já envia no formato que o Gráfico precisa
      alunosPorTurma: turmasStats
    }, 200);
  } catch (e) {
    console.error("Erro ao gerar m\xE9tricas:", e);
    return c.json({ erro: "Erro ao calcular m\xE9tricas no banco de dados." }, 500);
  }
});
servidor.get("/", (c) => c.json({
  mensagem: "Servidor backend est\xE1 conectado ao Cloudflare D1",
  status: "Online e Persistente"
}));
var index_default = servidor;
export {
